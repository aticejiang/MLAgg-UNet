...
###################################### It is a 2D method #########################################
class nnUNetTrainerUltraLightVMUNet(nnUNetTrainerNoDeepSupervision):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
                 device: torch.device = torch.device('cuda')):
        """used for debugging plans etc"""
        super().__init__(plans, configuration, fold, dataset_json, unpack_dataset, device)
        ### Some hyperparameters for you to fiddle with
        self.initial_lr = 1e-3          # from source code     #1e-4
        self.weight_decay = 1e-2        # from source code     #1e-3
    ...
    def build_network_architecture(plans_manager: PlansManager,
        ...
        model = UltraLight_VM_UNet(num_classes=model_config['num_classes'],
...
class UltraLight_VM_UNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # return torch.sigmoid(out0)            # source code
        return out0