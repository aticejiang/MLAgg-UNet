...
class nnUNetTrainerTransUNet(nnUNetTrainerNoDeepSupervision):
    ...
    def build_network_architecture(plans_manager: PlansManager,
        ...
        config_vit.n_skip = 3
        config_vit.img_size = configuration_manager.patch_size
        if 'R50-ViT-B_16'.find('R50') != -1:
            config_vit.patches.grid = (int(configuration_manager.patch_size[0] / 16), int(configuration_manager.patch_size[1] / 16))
...
class ResNetV2(nn.Module):
    ...
    def forward(self, x):
        features = []
        b, c, in_size_0, in_size_1 = x.size()
        x = self.root(x)
        features.append(x)
        x = nn.MaxPool2d(kernel_size=3, stride=2, padding=0)(x)
        for i in range(len(self.body)-1):
            x = self.body[i](x)
            right_size_0 = int(in_size_0 / 4 / (i+1))
            right_size_1 = int(in_size_1 / 4 / (i+1))
            if (x.size()[2] != right_size_0) or (x.size()[3] != right_size_1):
                pad = right_size_0 - x.size()[2]
                assert pad < 3 and pad > 0, "x {} should {}".format(x.size(), right_size_0)
                feat = torch.zeros((b, x.size()[1], right_size_0, right_size_1), device=x.device)
...
class DecoderCup(nn.Module):
    ...
    def forward(self, hidden_states, features=None):
        ...
        h, w = self.config.img_size[0]//self.config.patch_size, self.config.img_size[1]//self.config.patch_size
...
if __name__ == '__main__':
    ...
    config_vit.n_skip = 3
    config_vit.img_size = [320,384]
    if 'R50-ViT-B_16'.find('R50') != -1:
        config_vit.patches.grid = (int(320 / 16), int(384 / 16))
    model = VisionTransformer(config_vit,
                              in_channels=1,
                              img_size=[320,384],
                              num_classes=14)
