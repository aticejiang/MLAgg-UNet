...
class MedNeXtDownBlock(MedNeXtBlock):
    ...
    def __init__(self, in_channels, out_channels, exp_r=4, kernel_size=7, 
                do_res=False, norm_type = 'group', dim='3d', grn=False, stride: int or list=2):
...
class MedNeXtUpBlock(MedNeXtBlock):
    ...
    def __init__(self, in_channels, out_channels, exp_r=4, kernel_size=7, 
        ...
        if isinstance(self.stride, int):
            if dim == '2d':
                self.stride = [stride for i in range(2)]
            if dim == '3d':
                self.stride = [stride for i in range(3)]
        ...
        if isinstance(stride, list) and (1 in stride):
            kernel_size = [1 if i == 1 else kernel_size for i in stride]
    ...
    def forward(self, x, dummy_tensor=None):
        ...
        padding = [1 if s != 1 else 0 for s in self.stride]
        
        if self.dim == '2d':
            x1 = torch.nn.functional.pad(x1, (padding[1],0,padding[0],0))
        elif self.dim == '3d':
            x1 = torch.nn.functional.pad(x1, (padding[2],0,padding[1],0,padding[0],0))
        
        if self.resample_do_res:
            res = self.res_conv(x)
            if self.dim == '2d':
                res = torch.nn.functional.pad(res, (padding[1],0,padding[0],0))
            elif self.dim == '3d':
                res = torch.nn.functional.pad(res, (padding[2],0,padding[1],0,padding[0],0))
            x1 = x1 + res
...
if __name__ == "__main__":
    ...
    network = MedNeXtUpBlock(in_channels=12, out_channels=12, do_res=True, grn=False, norm_type='group', stride=[1,2,2]).cuda()
    # network = LayerNorm(normalized_shape=12, data_format='channels_last').cuda()
    # network.eval()
    with torch.no_grad():
        print(network)
        x = torch.zeros((2, 12, 20, 64, 64)).cuda()
        print(network(x).shape)
