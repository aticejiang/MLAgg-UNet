...
label_tolerance = OrderedDict({'Spleen':3, 'RK':3, 'LK':3, 'Gallbladder': 2,
                               'Esophagus':3, 'Liver': 5, 'Stomach': 5, 'Aorta': 2,
                               'IVC':2, 'PVSV':2, 'Pancreas':5, 'RAG':2, 'LAG':2,})
...
for name in tqdm(filenames):
    ...
    for i, organ in enumerate(label_tolerance.keys(),1):
        if np.sum(gt_data==i)==0 and np.sum(seg_data==i)==0:
            ...
            if i==8 or i==9 or i==5: # for Aorta, IVC, and Esophagus, only evaluate the labelled slices in ground truth
...
dataframe.to_csv(save_path + '/NSD.csv', index=False)
...
# 打开一个文件以写入数据
with open(save_path + '/NSD_avg.txt', 'w', encoding='utf-8') as file:
    file.write(f"{'NSD_avg'} = {round(case_avg_NSD, 4)}\n")  # 使用f-string格式化字符串
    file.write(f"{'NSD_avg_all'} = {round(case_avg_NSD.mean(), 4)}\n")  # 使用f-string格式化字符串
